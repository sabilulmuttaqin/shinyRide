    ...
    <main
        ...
        <section id="Store-list" class="flex flex-col gap-6 px-4 mt-[30px]">
            @forelse ($stores as $store)
                <a href="{{ route('pages.detail', $store->slug) }}" class="card">
                    <div
                        ...
                        <div class="w-full h-[120px] flex shrink-0 overflow-hidden relative">
                            ...
                            @else
                                <p
                                    class="rounded-full p-[6px_10px] bg-red-500 w-fit h-fit font-bold text-[10px] leading-[15px] text-white absolute top-4 right-4">
                                    CLOSED</p>
                            @endif
                        ...
                        <div class="flex items-center justify-between gap-4 px-4">
                            ...
                            <div class="rating flex flex-col gap-[6px]">
                                <div class="flex items-center justify-end text-right gap-[6px]">
                                    <div class="w-[76px] h-[18px] flex shrink-0">
                                        @for ($i = 0; $i < $store->rating; $i++)
                                            <img src="{{ asset('assets/images/icons/Star 1.svg') }}" alt="verified">
                                        @endfor
                                    </div>
                                    <h1 class="font-semibold w-fit">{{ $store->rating }}</h1>
                                </div>
